...
document.querySelector('#postForm').addEventListener('submit', e => {
    ...
    async function createPost() {
        ...
        if(session_id === post.user_id){
            delete_post_html = '<button class="remove-btn" onclick="removeMyPost(this)">Obrisi objavu</button>'
        }
        ...
        document.querySelector('#allPostsWrapper').innerHTML = `<div class="single-post" data-post_id="${post.id}">
                                                                ...
                                                                <div class="post-comments">
                                                                        <form>
                                                                            ...
                                                                            <button onclick="commentPostSubmit(event)">Objavi</button>
                                                                        </form>
                                                                </div>
                                                                </div>
                                                                ` + html;
        ...
        async function getPostUser(){
            ...
            if(comments.length > 0){
                comments.forEach(comment => {
                    comments_html += `<div class="single-comment">Autor: <br> ${comment.content}</div>`
                });
            }
